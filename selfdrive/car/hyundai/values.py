...
class CAR(Platforms):
  ...
  # Kisa
  AVANTE_AD = HyundaiPlatformConfig(
    "HYUNDAI AVANTE (AD)",
    HyundaiCarInfo("Hyundai Avante", "All", car_parts=CarParts.common([CarHarness.hyundai_b])),
    CarSpecs(mass=1250, wheelbase=2.7, steerRatio=15.4),
    flags=HyundaiFlags.LEGACY | HyundaiFlags.CLUSTER_GEARS | HyundaiFlags.MIN_STEER_32_MPH,
  )
  AVANTE_CN7 = HyundaiPlatformConfig(
    "HYUNDAI AVANTE (CN7)",
    HyundaiCarInfo("Hyundai Avante 2021", "All", car_parts=CarParts.common([CarHarness.hyundai_k])),
    CarSpecs(mass=1225, wheelbase=2.72, steerRatio=12.9, tireStiffnessFactor=0.65),
    flags=HyundaiFlags.CHECKSUM_CRC8,
  )
  AVANTE_HEV_CN7 = HyundaiPlatformConfig(
    "HYUNDAI AVANTE HYBRID (CN7)",
    HyundaiCarInfo("Hyundai Avante Hybrid 2021", "All", car_parts=CarParts.common([CarHarness.hyundai_k])),
    CarSpecs(mass=1335, wheelbase=2.72, steerRatio=12.9),
    flags=HyundaiFlags.CHECKSUM_CRC8, HyundaiFlags.HYBRID,
  )
  I30_PD = HyundaiPlatformConfig(
    "HYUNDAI I30 (PD)",
    HyundaiCarInfo("Hyundai I30", car_parts=CarParts.common([CarHarness.hyundai_e])),
    CarSpecs(mass=1225, wheelbase=2.72, steerRatio=12.9, tireStiffnessFactor=0.65),
    flags=HyundaiFlags.LEGACY | HyundaiFlags.CLUSTER_GEARS | HyundaiFlags.MIN_STEER_32_MPH,
  )
  GRANDEUR_IG = HyundaiPlatformConfig(
    "HYUNDAI GRANDEUR (IG)",
    HyundaiCarInfo("Hyundai Grandeur IG", "All", car_parts=CarParts.common([CarHarness.hyundai_c])),
    CarSpecs(mass=1560, wheelbase=2.845, steerRatio=14.5),
    flags=HyundaiFlags.CLUSTER_GEAR,
  )
  GRANDEUR_HEV_IG = HyundaiPlatformConfig(
    "HYUNDAI GRANDEUR HYBRID (IG)",
    HyundaiCarInfo("Hyundai Grandeur IG Hybrid", "All", car_parts=CarParts.common([CarHarness.hyundai_c])),
  ...
  NEXO_FE = HyundaiPlatformConfig(
    "HYUNDAI NEXO (FE)",
    HyundaiCarInfo("Hyundai NEXO", "All", car_parts=CarParts.common([CarHarness.hyundai_h])),
    CarSpecs(mass=1885, wheelbase=2.79, steerRatio=14.2, tireStiffnessFactor=0.385),
    flags=HyundaiFlags.EV,
  )
  KIA_K8_GL3 = HyundaiCanFDPlatformConfig(
    "KIA K8 GL3 1ST GEN",
    HyundaiCarInfo("Kia K8 GL3 (with HDA II) 2023", car_parts=CarParts.common([CarHarness.hyundai_k])),
    CarSpecs(mass=1642, wheelbase=2.895, steerRatio=13.27),
  )
  K3_BD = HyundaiPlatformConfig(
    "KIA K3 (BD)",
    HyundaiCarInfo("Kia K3 2018-21", car_parts=CarParts.common([CarHarness.hyundai_g])),
    CarSpecs(mass=1260, wheelbase=2.7, steerRatio=13.75, tireStiffnessFactor=0.5)
  )
  K5_JF = HyundaiPlatformConfig(
    "KIA K5 (JF)",
    HyundaiCarInfo("Kia K5 2021-22", "Advanced Smart Cruise Control",
  ...
  MOHAVE_HM = HyundaiPlatformConfig(
    "KIA MOHAVE (HM)",
    HyundaiCarInfo("Kia Mohave 2019"),
    CarSpecs(mass=2285, wheelbase=2.895, steerRatio=14.25),
  )
  GENESIS_DH = HyundaiPlatformConfig(
    "GENESIS (DH)",
    HyundaiCarInfo("Genesis 2015-2016", "All", car_parts=CarParts.common([CarHarness.hyundai_j])),
